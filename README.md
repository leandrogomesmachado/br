...
Para I/O de arquivo, BR expoe quatro funcoes embutidas que envolvem diretamente as syscalls Linux: `abrir_arquivo(caminho, flags, modo)` retorna um descritor inteiro (ou um valor negativo `-errno` em erro); `ler_bytes(fd, buf, n)` e `escrever_bytes(fd, buf, n)` movem ate `n` bytes entre o descritor e um buffer apontado por `buf`, devolvendo o numero de bytes efetivamente transferidos (ou negativo em erro); e `fechar_arquivo(fd)` libera o descritor. As constantes de `flags` (como `O_RDONLY = 0`, `O_WRONLY|O_CREAT|O_TRUNC = 577`) e de `modo` (como `0644 = 420`) seguem os valores POSIX e ainda sao escritas como literais inteiros, ja que esta versao nao tem `enum`/`#define`. Programas tambem podem escrever em `stderr` com `escrever_erro(literal_de_string)`, analogo a `escrever_texto` mas direcionado ao descritor 2.

Como literais de string ficam em `.rodata` com `'\0'` implicito ao final, qualquer literal pode ser atribuido a um `caractere *` ou passado como argumento de tipo `caractere *` (ex.: `caractere *p = "ola"`). Para acessar bytes individuais, use `ler_byte(ptr, i)` (devolve um `inteiro` entre 0 e 255) e `escrever_byte(ptr, i, valor)` (grava o byte baixo de `valor`); a indexacao classica `ptr[i]` continua disponivel mas estende-se em passos de 8 bytes (pois cada `caractere` ocupa um slot de 8 bytes), enquanto `ler_byte`/`escrever_byte` operam em bytes reais e contiguos, indispensaveis para tratar streams binarios e codigo fonte.

O programa pode tambem inspecionar a linha de comando: `numero_argumentos()` retorna a contagem `argc` recebida pelo kernel e `argumento(i)` devolve um `caractere *` para o i-esimo argumento (com `argumento(0)` sendo o caminho do executavel). E para encerrar imediatamente o processo com um codigo de saida arbitrario, use `sair(codigo)` (syscall `exit_group`), util tanto em diagnosticos de erro do compilador BR quanto em qualquer programa que precise abortar antes do retorno de `principal`.

Para invocar programas externos (por exemplo o montador `as` e o vinculador `ld`), BR oferece tres builtins envolvendo as syscalls `fork`, `execve` e `wait4`: `bifurcar()` cria um processo filho retornando 0 ao filho, o pid ao pai e um valor negativo `-errno` em erro; `executar(caminho, argv)` substitui o processo atual pelo programa apontado por `caminho`, com `argv` sendo um `caractere **` terminado em `nulo` (envp e' passado como `NULL`); e `aguardar(pid)` bloqueia ate o filho identificado por `pid` terminar, retornando o pid aguardado em sucesso ou negativo em erro. Esse trio cobre o ciclo completo de fork-exec-wait, suficiente para que o brc futuro (escrito em BR) possa orquestrar a montagem e o link de seu proprio Assembly em executavel.

Variaveis globais sao declaradas no nivel do arquivo, fora de qualquer funcao, com a sintaxe `tipo nome [= literal];`. Sao suportados tipos escalares (`inteiro`, `caractere`) e ponteiros (`tipo *`, `caractere **`, `estrutura Foo *`). Vetores e estruturas-por-valor globais sao recusados nesta versao. Inicializadores precisam ser literais conhecidos em tempo de compilacao: literal inteiro com sinal opcional, literal de caractere, literal de string (que vira ponteiro para a area `.rodata`) ou `nulo`. Globais sem inicializador sao zero-inicializadas (o que equivale a `0` para inteiros/caracteres e a `nulo` para ponteiros). Variaveis locais com mesmo nome sombreiam a global dentro da funcao, seguindo o modelo classico de C. Internamente cada global e' emitida em `.data` com o simbolo `__br_g_<nome>` e acessada via enderecamento RIP-relativo, permitindo que o estado persista entre chamadas e seja compartilhado por todas as funcoes do programa - um requisito basico para escrever um compilador, lexer ou qualquer programa nao-trivial em BR.
